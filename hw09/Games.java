...
public class Games {
    ...
    public static void scrambler() { // create a method for scrambler
        ...
        Scanner myScanner = new Scanner(System.in); // create a scanner
        String character = myScanner.nextLine(); // accept the input
        int length = character.length(); // determine how many characters the word is
        int j = 0; // initialize variable
        // Create the Random generator
        Random randomGenerator = new Random();
        String word = ""; // initialize string
        while (j < 2) { // scramble the word twice
            word = ""; 
            // Use the nextInt() method of Random() class to generate a random integer 
            ...
            for (int i = 1; i < length; i++) {
                if (i == randomInt) { // determine the first letter
                    word += character.charAt(0);
                }
                else { // determine where the rest of the letters are placed, based on what the random generator chooses
                    word += character.charAt(i);
                }
            }
            j++; // insures that the while loop does not run infinitely
            character = word; // make the word scrambled the first time, the same word, so it can be scrambled again
        }
